## Эта страничка содержит описание наиболее часто употребляемых утилит для стеганографии (и частично форензики)
*****************************************************************
## Первичный анализ:
...
|-----|-------------|-------|
|file|Определет тип файла|file -filename|
|exiftool|Посмотреть основные метаданные (для графических файлов)|exiftool - filename |
|binwalk|Показывает встроенные контейнеры| binwalk -filename |
|strings | Показывает все текстовые строки | strings -filename |
|foremost | Извлекает встроенные контейнеры | foremost -filename |
|pngcheck | Проверяет файлы формата PNG | pngcheck вЂ“options -filename |
|ffmpeg | Проверяет целостность аудио файлов | ffmpeg вЂ“options -filename |
